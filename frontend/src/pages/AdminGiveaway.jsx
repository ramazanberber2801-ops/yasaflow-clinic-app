...
function filterEntries(entries, { uniqueUsers, requiredWord, excludedUsers }) {
  ...
  const word = requiredWord.trim().toLowerCase();
  const filtered = entries.filter((entry) => {
    if (blocked.has(entry.username.toLowerCase())) return false;
    if (word && !entry.comment.toLowerCase().includes(word)) return false;
    return true;
  });
...
function securePick(items, count) {
  ...
  while (pool.length && selected.length < count) {
    ...
    selected.push(pool.splice(values[0] % pool.length, 1)[0]);
  }
  return selected;
}
...
function drawCanvasFrame(ctx, canvas, { title, subtitle, names, footer, progress = 1 }) {
  ...
  for (let index = 0; index < 24; index += 1) {
    const x = ((index * 173) % width) + Math.sin(progress * 8 + index) * 30;
    const y = ((index * 251) % height) + Math.cos(progress * 7 + index) * 40;
    ctx.beginPath();
    ctx.arc(x, y, 8 + (index % 4) * 4, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  ctx.fillText("YASAFLOW", width / 2, 150);

  ctx.fillStyle = "#FFFFFF";
  ctx.font = "700 66px serif";
  ctx.fillText(title, width / 2, 310);

  ctx.fillStyle = "rgba(255,255,255,0.72)";
  ctx.font = "32px sans-serif";
  ctx.fillText(subtitle, width / 2, 375);

  const startY = height / 2 - ((names.length - 1) * 82) / 2;
  names.forEach((name, index) => {
    ctx.fillStyle = "#FFFFFF";
    ctx.font = "700 56px serif";
    ctx.fillText(`@${name}`, width / 2, startY + index * 82);
  });

  ctx.fillStyle = "rgba(255,255,255,0.68)";
  ctx.font = "28px sans-serif";
  ctx.fillText(footer, width / 2, height - 140);
}
...
export default function AdminGiveaway() {
  ...
  const startSession = async () => {
    if (!/^https?:\/\/(www\.)?instagram\.com\/(p|reel)\//i.test(instagramUrl.trim())) {
      return toast.error("Lim inn en gyldig offentlig Instagram-lenke");
    }
    if (!participants.length) return toast.error("Legg inn minst én kvalifisert deltaker");
    if (winnerCount + reserveCount > participants.length) {
      return toast.error("Det er færre deltakere enn vinnere og reserver til sammen");
    }
  ...
  const runDraw = async () => {
    ...
    for (let index = 0; index < 28; index += 1) {
      const values = new Uint32Array(1);
      crypto.getRandomValues(values);
      setRollingName(participants[values[0] % participants.length]);
    ...
    try {
      ...
      toast.error(error.message || "Trekningen ble gjennomført, men resultatet kunne ikke lagres midlertidig");
    } finally {
      setDrawing(false);
    }
  };
  ...
  const downloadResult = () => {
    ...
    const content = [
      ...
      `Kvalifiserte deltakere: ${participants.length}`,
      ...
      "Resultatet er lagret lokalt. Yasaflow beholder ingen filkopi.",
    ].filter(Boolean).join("\n");
  ...
  const createLocalVideo = async () => {
    ...
    if (!window.MediaRecorder || !HTMLCanvasElement.prototype.captureStream) {
      return toast.error("Nettleseren støtter ikke lokal video. Bruk Chrome eller Edge.");
    }
    ...
    try {
      ...
      await new Promise((resolve) => {
        const render = (now) => {
          ...
          const reveal = elapsed < 1300
            ? [String(Math.max(1, 3 - Math.floor(elapsed / 430)))]
            : winners;
          drawCanvasFrame(ctx, canvas, {
            title: elapsed < 1300 ? "Trekningen starter" : "Gratulerer!",
            subtitle: elapsed < 1300 ? "Rettferdig og tilfeldig trekning" : `${participants.length} kvalifiserte deltakere`,
            names: reveal,
            footer: "Privacy First Giveaway • ingen fil lagres hos Yasaflow",
            progress,
          });
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <section className="mt-5 rounded-[32px] bg-[#2C2A26] p-7 text-white shadow-xl">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-6 text-white/65">
            Ingen Instagram-pålogging. Data slettes automatisk etter 12 timer. Resultat og video lagres bare lokalt på enheten din.
          </p>
        </section>
        ...
        <section className="mt-6 rounded-3xl border border-[#EBE5DC] bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 flex items-center justify-between gap-3">
            <div>
              <label className="block text-sm font-medium">Deltakere eller kommentarer</label>
              <p className="mt-1 text-xs text-[#6B655B]">Format: <strong>brukernavn | kommentar</strong>. Kommentar er valgfritt.</p>
            </div>
            <button onClick={() => fileInputRef.current?.click()} className="inline-flex shrink-0 items-center gap-2 rounded-full border border-[#DED6CA] px-3 py-2 text-xs">
              <Upload size={15} /> Importer fil
            </button>
          ...
          <textarea
            ...
            rows={10}
            placeholder={"seldaesthetic | jeg deltar\nmedina | med min venn\nklinikkkunde"}
            className="mt-2 w-full rounded-2xl border border-[#DED6CA] px-4 py-3 outline-none focus:border-[#C5A059]"
          />
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label className="rounded-2xl bg-[#F7F3EC] p-4 text-sm">
              Kommentar må inneholde
              <input value={requiredWord} onChange={(event) => setRequiredWord(event.target.value)} placeholder="Valgfritt ord" className="mt-2 w-full rounded-xl border border-[#DED6CA] bg-white px-3 py-2" />
            </label>
            <label className="rounded-2xl bg-[#F7F3EC] p-4 text-sm">
              Antall vinnere
              <input type="number" min="1" max="10" value={winnerCount} onChange={(event) => setWinnerCount(Math.max(1, Math.min(10, Number(event.target.value) || 1)))} className="mt-2 w-full rounded-xl border border-[#DED6CA] bg-white px-3 py-2" />
            </label>
            <label className="rounded-2xl bg-[#F7F3EC] p-4 text-sm">
              Antall reserver
              <input type="number" min="0" max="10" value={reserveCount} onChange={(event) => setReserveCount(Math.max(0, Math.min(10, Number(event.target.value) || 0)))} className="mt-2 w-full rounded-xl border border-[#DED6CA] bg-white px-3 py-2" />
            </label>
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-4 text-sm text-[#6B655B]">
            <span className="inline-flex items-center gap-2"><Users size={17} /> {qualifiedEntries.length} kvalifiserte</span>
            <span>{parsedEntries.length - qualifiedEntries.length} filtrert bort</span>
          </div>

          <button onClick={startSession} disabled={loading || drawing} className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-[#2C2A26] px-5 py-3.5 text-sm text-white disabled:opacity-50">
        ...
        {session && (
          <section className="mt-6 overflow-hidden rounded-3xl border border-[#EBE5DC] bg-white p-5 text-center shadow-sm">
            ...
            <p className="mt-2 text-sm text-[#6B655B]">Midlertidige data utløper {new Date(session.expires_at).toLocaleString("no-NO")}.</p>

            <AnimatePresence mode="wait">
              {countdown !== null && (
                <motion.div key={countdown} initial={{ scale: 0.35, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 1.6, opacity: 0 }} className="my-8 font-serif-display text-8xl text-[#C5A059]">
                  {countdown}
                </motion.div>
              )}
              {rollingName && countdown === null && (
                <motion.div key={rollingName} initial={{ y: 24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -24, opacity: 0 }} className="my-8 rounded-3xl bg-[#F7F3EC] p-7 font-serif-display text-4xl">
                  @{rollingName}
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {!drawing && (
              <button onClick={runDraw} className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-[#C5A059] px-5 py-4 text-base font-medium text-white">
                {winners.length ? <RefreshCw size={18} /> : <Sparkles size={18} />}
                {winners.length ? "Trekk på nytt" : `Trekk vinner${winnerCount > 1 ? "e" : ""}`}
              </button>
            )}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-5 rounded-3xl bg-[#F7F3EC] p-6">
                ...
                {reserves.length > 0 && (
                  <div className="mt-5 border-t border-[#DED6CA] pt-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-[#6B655B]">Reserve{reserves.length > 1 ? "r" : ""}</p>
                    <div className="mt-2 space-y-1 text-lg">{reserves.map((reserve) => <div key={reserve}>@{reserve}</div>)}</div>
                  </div>
                )}
                <div className="mt-5 flex flex-wrap justify-center gap-3">
                  <button onClick={downloadResult} className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-3 text-sm shadow-sm"><Download size={17} /> Lagre resultat</button>
                  <button onClick={createLocalVideo} disabled={videoLoading} className="inline-flex items-center gap-2 rounded-full bg-[#2C2A26] px-4 py-3 text-sm text-white disabled:opacity-50"><Film size={17} /> {videoLoading ? "Lager video..." : "Lag 9:16-video"}</button>
                </div>
              </motion.div>
            )}
